...
spc.rmsexv{i} = feGetRep(fe,'vox rmse');
...
spc.rrmse{i}  = feGetRep(fe,'vox rmse ratio');
...
spc.w{i}      = feGet(fe,'fiber weights');
...
s.coords{i} = feGet(fe,'roi coords');
clear fe
end
...
etc.coordsIdx = ismember(e.coords,s.coords{3},'rows');
etc.coords    = e.coords(etc.coordsIdx,:);
spc.coordsIdx{3}  = ismember(s.coords{3},etc.coords,'rows');
spc.coords{3}     = s.coords{3}(spc.coordsIdx{3},:);
etc_rrmse_comparison      = etc.rrmse(etc.coordsIdx);
spc_rrmse_comparison       = spc.rrmse{3}(spc.coordsIdx{3});
...
minsca = 0.5;
scabin = 0.0125;
[ymap_prep,x]  = hist3([spc_rrmse_comparison;etc_rrmse_comparison]',{[(minsca-scabin):scabin:(maxsca+scabin)], [(minsca-scabin):scabin:(maxsca+scabin)]});

% Clip the first and last row in histogram (which includes value out of the
% range)
...
set(gca, ...
    'xlim',[1 size(ymap,1)],...
    'ylim',[1 size(ymap,2)], ...
    'ytick',[1 (1+ size(ymap,1))/2 size(ymap,1)], ...
    'xtick',[1 (1+ size(ymap,2))/2 size(ymap,2)], ...
    'yticklabel',[x{1}(end-1) x{1}(round((end+1)/2)) x{1}(2)], ...
    'xticklabel',[x{1}(2)   x{1}(round((end+1)/2)) x{1}(end-1)], ...
    'tickdir','out','ticklen',[.025 .05],'box','off', ...
    'fontsize',16,'visible','on')
hold on
plot3([2 size(ymap,1)],[size(ymap,1) 1],[max(ymap(:)) max(ymap(:))],'k-','linewidth',1)
ylabel('SPC1mm_{Rrmse}','fontsize',16)
xlabel('ETC_{Rrmse}','fontsize',16)
cb = colorbar;
    tck = get(cb,'ytick');
